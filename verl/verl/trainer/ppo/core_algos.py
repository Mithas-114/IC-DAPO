...
def compute_cispo_loss(
    ...
    ratio = ratio.detach()
    importance_sampling_weight = torch.clamp(ratio, max=1 + cliprange_high, min=1 - cliprange_low)
...
def compute_policy_loss_gspo(
    ...
    clip_ratio_low = cliprange_low if cliprange is not None else cliprange_low
    clip_ratio_high = cliprange_high if cliprange is not None else cliprange_high
