...
import re
# from .configuration_llama import KuaiYiMoeConfig
...
def convert_fsdp_checkpoints_to_hfmodels():
    ...
    # Define patterns for buffer parameters that should not be merged
    buffer_patterns = [
        r'.*\.expert_bias$',  # expert_bias buffers
        r'.*\.num_batches_tracked$',  # BatchNorm buffers
        r'.*\.running_mean$',  # BatchNorm buffers
        r'.*\.running_var$',  # BatchNorm buffers
        # Add more patterns as needed
    ]
    
    def is_buffer_param(key):
        """Check if a parameter is a buffer that should not be merged"""
    ...
    for key in sorted(state_dict):
        ...
        if is_buffer_param(key):
            # For buffer parameters, use the first shard as they should be identical
            state_dict[key] = state_dict[key][0]
            print(f"Using first shard for buffer parameter: {key}")
            continue
    ...
    # config = KuaiYiMoeConfig.from_pretrained(args.hf_model_path)
