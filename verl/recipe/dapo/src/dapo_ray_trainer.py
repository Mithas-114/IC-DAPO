            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    if "acc" in reward_extra_infos_dict:
                        ...
                        acc_array = np.array(reward_extra_infos_dict["acc"])
                        rollout_n = self.config.actor_rollout_ref.rollout.n 
                        num_problems = len(acc_array) // rollout_n  

                        acc_per_problem = np.zeros((num_problems, rollout_n))
                        for idx, acc_val in enumerate(acc_array):
                            problem_id = idx % num_problems
                            sample_id = idx // num_problems
                            acc_per_problem[problem_id, sample_id] = acc_val

                        # pass@1
                        metrics["train/pass@1"] = float(np.mean(np.mean(acc_per_problem, axis=1)))
                        
                        # pass@k
                        has_correct_per_problem = np.any(acc_per_problem == 1, axis=1)
                        metrics[f"train/pass@{rollout_n}"] = float(np.mean(has_correct_per_problem))


                    if is_finish_list is not None:
                        assert len(is_finish_list) == len(new_batch.non_tensor_batch["uid"])
                        kept_traj_idxs = []
                        ...
                        pre_len = len(new_batch)
                        new_batch = new_batch[kept_traj_idxs]
                        aft_len = len(new_batch)
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        if len(batch) < target_size:
                            ...
                            if max_num_gen_batches <= 0 or num_gen_batches < max_num_gen_batches:
                                ...
                                max_batch_size = (batch.batch['input_ids'].shape[0] // world_size) * world_size
